...
local icon_font = "JetBrainsMono Nerd Font 9"
local configuration_bin = gears.filesystem.get_configuration_dir() .. "bin/"
local margin_vert = 6

local function text_font_with_size(size)
    return beautiful.font:gsub("%d+$", size)
end

local function icon_font_with_size(size)
    return icon_font:gsub("%d+$", size)
end
...
local function create_icon_widget(text, args)
    ...
    return wibox.widget {
        text         = text,
        font         = icon_font_with_size(args.font_size or 9),
        align        = 'center',
        valign       = 'center',
...
local function create_icon_text_widget(args)
    return {
        {
            {
                {
...
mywidgets.container_kernel_widget = create_icon_text_widget({
...
mywidgets.container_uptime_widget = create_icon_text_widget({
    left_widget = create_icon_widget(""),
...
mywidgets.container_cpu_widget = create_icon_text_widget({
    left_widget = create_icon_widget("﬙"),
    right_widget = mycpu,
})
...
mywidgets.container_meminfo_widget = create_icon_text_widget({
    left_widget = create_icon_widget(""),
    icon_font = icon_font_with_size(10),
...
mywidgets.container_disk_widget = create_icon_text_widget({
    left_widget = create_icon_widget("", { font_size = 8 }),
    icon_font = icon_font_with_size(8),
    right_widget = mydisk,
})
...
mywidgets.container_traffic_widget = create_icon_text_widget({
    left_widget = create_icon_widget("李", { font_size = 11 }),
    icon_font = icon_font_with_size(10),
...
mywidgets.container_clock_widget = create_icon_text_widget({
    left_widget = create_icon_widget("", { font_size = 8 }),
